...
def build_init_ids_multi_start(
    ...
    *,
    ...
    relocate_rounds: int = 1,
    local_improve_top_k: int = 2,
) -> List[int]:
    """Create several initial solutions and pick the best by cost.

    변경점:
    - 후보는 기존처럼 여러 개 생성
    - 먼저 LS 없이 cost 기준으로 정렬
    - 상위 local_improve_top_k개 후보에만 relocate LS 적용
    """
    id2idx, _ = build_id_maps(address_list)
    ...
    normalized_scored: List[tuple[float, List[int]]] = []
    ...
    for o in candidates:
        ...
        normalized_scored.append((c, order))

    normalized_scored.sort(key=lambda x: (x[0], x[1]))

    # baseline: LS 없이도 가장 좋은 후보를 우선 best로 둠
    best_cost, best_order = normalized_scored[0]

    if not use_relocate_ls:
        return best_order

    top_k = max(1, int(local_improve_top_k))
    top_k = min(top_k, len(normalized_scored))

    for idx in range(top_k):
        _, order0 = normalized_scored[idx]

        improved_order = _local_improve_relocate(
            list(order0),
            start_id=start_id,
            end_id=end_id,
            id2idx=id2idx,
        ...
        improved_cost = calc_cost(improved_order, id2idx, mat)
